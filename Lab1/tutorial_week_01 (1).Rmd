...
\begin{eqnarray*}
\text{height (cm)} &=& \text{height (ft)} \times 0.3048 \\
...
> 5. Use `ggplot` and `geom_point` to make a scatter plot of weight (in kg) by height (in cm), and size by BMI. Use the `alpha` command add transparency.  Comment on the plot.  
